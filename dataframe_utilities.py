...
def df_by_file_id(file_id, sheet_index=None):
    ...
    excel_content_types = ['application/vnd.ms-excel','text/html','application/octet-stream','application/vnd.openxmlformats-officedoc',
                        'application/vnd.oasis.opendoc', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet']
    with Database('spreadsheets.db') as db:
        ...
        if (res[0][1].lower().endswith('.csv') or res[0][2].startswith(tuple(csv_content_types))and not res[0][1].lower().endswith('.xls')):
            ...
            try:
                ...
                for line in lines:
                    print(line)
        elif res[0][1].lower().endswith(('.xls', '.xlsx', '.ods')) or res[0][2].startswith(tuple(excel_content_types)):
