...
def update_db(cursor, result):
    ...
    if result[1] == "SUCCESS":
        ...
        filename = result[3]
        cursor.execute(
            """UPDATE files SET file_name = ?, http_response_code = ?, response_headers = ?,
        ...
        r = result[2]
        cursor.execute(
            "UPDATE files SET file_name = NULL, http_response_code = ?, status_reason = ?, error_message = NULL, last_modified = ? WHERE url = ?",
...
def download_batch(pool, con, batchsize):
    ...
    for row in cursor.execute(
        "SELECT url FROM files WHERE file_name IS NULL LIMIT 100"
    ):
        print(row)

    # Only retry failed attempts if they failed more than 3,600 seconds (an hour) ago
    old_error_time_limit = (
        datetime.datetime.now(datetime.UTC) - epoch
    ).total_seconds() - 3600
    ...
    batch = cursor.execute(
        "SELECT file_id, url FROM files WHERE file_name IS NULL AND (last_modified < ? OR last_modified IS NULL) ORDER BY random() LIMIT ?",
        (old_error_time_limit, batchsize),
    ).fetchall()
    if len(batch) == 0:
...
if __name__ == "__main__":
    ...
    print("Waiting for pool shutdown...")
    pool.close()
    pool.join()
